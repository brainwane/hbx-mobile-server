...
class ServerConfig
	def iam_home
		'https://preprod.dchealthlink.com'
	end

	def iam_login_form
		'https://preprod.dchealthlink.com/login-sso'
	end

	def iam_login
		'https://webpp.dchealthlink.com/oaam_server/loginAuth.do'
	end

	def iam_login_auth_jump
		'https://webpp.dchealthlink.com/oaam_server/authJump.do?jump=false'
	end

	def iam_challenge_user
		'https://webpp.dchealthlink.com/oaam_server/challengeUser.do'
	end
...
class VisitsController < ApplicationController
	...
	def answer_security_question
		...
		req = mechanize.post(@server_config.iam_challenge_user, form_values)
		code = req.code.to_i
		...
		render :json => {session_id: session_id, enroll_server: @server_config.enroll_url}
	end
	...
	def create
		...
		begin
			...
			security_question, cookies, hidden_values = do_login(email, password)
	...
	def do_login(email, password)
		...
		print "gem versions: \n"
		mechanize.agent.http.verify_mode = OpenSSL::SSL::VERIFY_NONE
		print "server: #{@server_config.iam_home}\n"
		req = mechanize.get(@server_config.iam_home)
		...
		req = mechanize.get(@server_config.iam_login_form)
		...
		req = mechanize.post(@server_config.iam_login, form_values)
		...
		sleep 3.0
		req = mechanize.get(@server_config.iam_login_auth_jump)
